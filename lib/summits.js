...
export async function detectSummits(activityId, polyline, startDate, db) {
  ...
  const { data: stored } = await db.from('summits').select('id, lat, lon').in('osm_id', osmIds)
